...
def submit():
    ...
    def highlight(sequence,summary,exon_utr,snp,bisulfite):
        ...
        start_utr_tag = '''<span style="font-style:italic">'''
        end_utr_tag = "</span>"
    ...
    ensembl_transcript_id = str(request.form['gene'])

    sequence = ensembl_sequence(ensembl_transcript_id)

    summary = ensembl_summary(ensembl_transcript_id)
    transcript_name = summary[0]['Associated Transcript Name']
    transcript_ID = summary[0]['Ensembl Transcript ID']
    transcript_status = summary[0]['Status (transcript)']
    transcript_source = summary[0]['Source (transcript)']
    transcript_type = summary[0]['Transcript type']
    transcript_chromosome = summary[0]['Chromosome Name']
    transcript_start_bp =  summary[0]['Transcript Start (bp)']
    transcript_end_bp = summary[0]['Transcript End (bp)']
    transcript_direction = summary[0]['Strand']
    ...
    return render_template('transcript.html',transcript_name = transcript_name,transcript_ID = transcript_ID,transcript_status = transcript_status  + ',' + transcript_source,transcript_type=transcript_type,transcript_strand = transcript_chromosome + ':' + transcript_start_bp + ':' + transcript_end_bp + ',' + transcript_direction,transcript = transcript,bisulfite = bisulfite)
